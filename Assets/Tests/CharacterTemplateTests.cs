...
public class CharacterTemplateTests
{
    ...
    {
        m_characterPrefab = Object.Instantiate(m_characterPrefab);
        m_characterPrefab.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("bandit");
        yield return new WaitForSeconds(0.1f);

        Assert.AreEqual(m_characterPrefab.GetComponent<SpriteRenderer>().sprite.name, "bandit");
    }
